...
def main(lookback_window_length: int, kernel: str, num_mixtures: int):
    ...
    all_processes = [
        f'python -m examples.cpd_quandl "{ticker}" "{os.path.join(CPD_QUANDL_OUTPUT_FOLDER(lookback_window_length, kernel), ticker + ".csv")}" "1990-01-01" "2021-12-31" "{lookback_window_length}" --kernel {kernel} --num_mixtures {num_mixtures}'
